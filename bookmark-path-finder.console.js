(async function () {
  ...
  const EXCLUDE_EXT =
    /\.(png|jpe?g|gif|webp|svg|ico|woff2?|ttf|otf|eot|mp4|mp3|pdf)(\?|#|$)/i;
  ...
  panel.innerHTML = `
    <div style="display:flex;gap:10px;align-items:center;justify-content:space-between;padding:12px 14px;border-bottom:1px solid rgba(255,255,255,.08);">
      <div style="display:flex;flex-direction:column;gap:2px;">
        <div style="font-weight:700;font-size:14px;">🔎 Bookmark Path Finder</div>
  ...
  function classify(path) {
    ...
    if (
      /\.(png|jpe?g|gif|webp|svg|ico|woff2?|ttf|otf|eot|mp4|mp3|pdf)(\?|#|$)/i.test(
        p
      ) ||
      p.includes("/assets") ||
      p.includes("/static")
      ...
      return "assets";
    if (
      /\.(js|mjs|cjs|css|map|json)(\?|#|$)/i.test(p) ||
      p.startsWith("./") ||
      p.startsWith("../")
    )
      return "imports";
  ...
  function isLikelyTextResource(url) {
    return !!url && !EXCLUDE_EXT.test(url);
  }
  ...
  function extractPaths(text) {
    ...
    for (const m of text.matchAll(/['"]((?:\/|\.\.\/|\.\/)[^'"]{1,300})['"]/g))
      out.add(m[1]);
    for (const m of text.matchAll(/`((?:\/|\.\.\/|\.\/)[^`]{1,300})`/g))
      out.add(m[1]);

    for (const m of text.matchAll(
      /\b(?:fetch|axios\.(?:get|post|put|delete|patch)|\$\.(?:get|post))\s*\(\s*([`'"])(.{1,300}?)\1/g
    ))
      out.add(m[2]);
    for (const m of text.matchAll(
      /\.open\s*\(\s*['"](GET|POST|PUT|DELETE|PATCH)['"]\s*,\s*([`'"])(.{1,300}?)\2/g
    ))
      out.add(m[3]);
    for (const m of text.matchAll(
      /\b(?:import\s+(?:[^'"]+\s+from\s+)?|require\s*\()\s*([`'"])(.{1,300}?)\1/g
    ))
      out.add(m[2]);
  ...
  async function fetchText(url) {
    ...
    try {
      ...
      if (!r.ok) return { ok: false, text: null };
      ...
      if (text.length > MAX_TEXT_SIZE) return { ok: false, text: null };
      return { ok: true, text };
    } catch {
      return { ok: false, text: null };
    } finally {
  ...
  function render(rows, currentTab, q) {
    ...
    if (currentTab !== "all")
      filtered = filtered.filter((r) => r.type === currentTab);
    if (query)
    ...
    for (const r of filtered) {
      ...
      tr.innerHTML = `
        ...
        <td style="padding:10px;word-break:break-word;font-weight:650;">${escapeHtml(
          r.path
        )}</td>
        <td style="padding:10px;border-radius:0 12px 12px 0;opacity:.85;word-break:break-word;">${escapeHtml(
          r.source
        )}</td>
      `;
  ...
  for (const btn of panel.querySelectorAll(".__ps_tab__")) {
    btn.onclick = () => {
      for (const b of panel.querySelectorAll(".__ps_tab__"))
  ...
  panel.querySelector("#__ps_copy__").onclick = async () => {
    try {
      ...
      setTimeout(() => (statusEl.textContent = "Done ✅"), 900);
    } catch {
  ...
  statusEl.textContent = "Scanning DOM/HTML…";
  const htmlText = document.documentElement
    ? document.documentElement.outerHTML
    : "";
  for (const p of extractPaths(htmlText))
    uniqPush(pathToSources, p, "DOM/HTML");

  const targets = resources.filter(isLikelyTextResource);
  statusEl.textContent = `Scanning ${targets.length} text-like resources…`;
  ...
  async function worker() {
    while (idx < targets.length) {
      ...
      statusEl.textContent = `OK: ${okCount}, Failed: ${failCount}, Paths: ${pathToSources.size}`;
    }
  }

  await Promise.all(
    Array.from({ length: Math.min(MAX_CONCURRENCY, targets.length) }, () =>
      worker()
    )
  );
  ...
  rows.sort(
    (a, b) => order[a.type] - order[b.type] || a.path.localeCompare(b.path)
  );
  ...
  statusEl.textContent = `Done ✅ — Found ${rows.length} unique paths`;
